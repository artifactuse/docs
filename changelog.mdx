...
<Update label="v0.4.1" description="Fix duplicate image in agent message">

## Bug Fixes

- **Duplicate image rendered in agent messages** — When a message contained a raw image URL (e.g. `https://example.com/photo.png`), the image processor was rendering the image twice: once from the auto-linked `<a>` tag and a second time from the URL leaking as plain text into the image caption div. The fix strips raw URLs from caption text and adds a re-protection step between the two image-processing passes so the raw-URL regex can never double-process content created in the same pass.

</Update>

<Update label="v0.4.0" description="Binary Workspace File Preview">

## New Features

- **Workspace file preview** — Binary files from a workspace (images, audio, video, PDFs, fonts) can now be displayed in the panel by passing base64 content in an artifact. The panel converts them to blob URLs and renders the appropriate element.
- **Six preview categories** — `image` (`<img>`), `audio` (`<audio controls>`), `video` (`<video controls>`), `pdf` (embedded viewer), `font` (`@font-face` glyph preview), `binary` (hex dump fallback).
- **Automatic blob URL management** — The panel creates and revokes `Object URL`s internally. No cleanup needed by the caller.
- **Extensible** — `BINARY_EXTS`, `BINARY_EXT_LANGUAGE`, `getMimeType`, `base64ToObjectUrl`, and `revokeBlobUrl` are exported as public API so developers can register additional extensions.
...
const ext = 'png';
sdk.createArtifact(base64Content, BINARY_EXT_LANGUAGE[ext], {
  title: 'logo.png',
  fileExtension: ext,
});
```
...
- Added [Workspace Files](/artifacts/files) page

</Update>

<Update label="v0.3.0" description="Console Output & Log Filtering">
...
- Added [Console Output](/artifacts/code#console-output) section to Code artifacts page

</Update>

<Update label="v0.2.2" description="Multi-Tab Artifact Panel">
...
- Added [Multi-Tab](/components/panel#multi-tab) section to ArtifactusePanel page

</Update>

<Update label="v0.2.1" description="Configurable Panel Width & Split Position">
...
- Added [Props](/components/panel#props) section to ArtifactusePanel page
- Updated [Theming](/customization/theming#configuration) with layout config options

</Update>

<Update label="v0.2.0" description="All Panels Now Free & Open Source">
...
<Update label="v0.1.31" description="CSV/TSV Spreadsheet Panel">
...
- Added [CSV / TSV](/artifacts/code#csv--tsv) section to Code artifacts page
- Updated panel README with sheet-panel docs

</Update>

<Update label="v0.1.30" description="Exclude Languages">
...
- Added [Exclude Languages](/components/agent-message#exclude-languages) section
...
- Added [Non-Clickable Short Code](/components/agent-message#non-clickable-short-code) and [Custom Action Labels](/components/agent-message#custom-action-labels) sections
